...
export const getList = async (req, res) => {
  try {
    ...
    var response = lists({ name: listType });
...
export const getFilteredList = async (req, res) => {
  try {
    const { listType, filter } =  req.params;

    var response = lists({ name: listType, filter: filter });
...
const lists = (params) => {
  switch (params.name) {
    ...
    case 'order_types': {
      ...
      return [
        { V_Type: 'SORD', Description: 'Sales Order'},
        { V_Type: 'ORD', Description: 'Orders'}
      ];
    }
    case 'party_list': {
      var filter = params.filter;
